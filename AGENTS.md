...
- **Also live (Sept 11)** — `canvaslms.com` (Instructure Canvas SSO: `sso.canvaslms.com`) and `mcas.ms` (Microsoft Defender for Cloud Apps conditional-access proxies: schools route sign-in through hosts like `tenant.access.mcas.ms`; matches Microsoft's documented `*.mcas.ms` allowance). Both added as parents, so all subdomains are covered in both layers.
