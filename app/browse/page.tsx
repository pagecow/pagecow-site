          ...
          <nav className="sticky top-[57px] flex flex-col gap-0.5 overflow-y-auto p-3 max-h-[calc(100vh-57px)]">
            {allCategories.map((cat) => (
              <button
                key={cat}
                onClick={() => handleCategoryChange(cat)}
                className={`group flex items-start gap-2.5 rounded-lg px-3 py-2 text-left text-sm transition-all ${
                  category === cat
              ...
              >
                ...
                <span
                  className={`ml-auto shrink-0 self-start pt-px text-[11px] tabular-nums ${
                    category === cat
                      ? "text-brand/70"
